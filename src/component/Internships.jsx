...
const InternshipCard = memo(({ internship, stackIndex, onSwipe }) => {
  ...
  return (
    <motion.div
      ...
      className={`absolute w-full max-w-4xl will-change-transform ${
        isFront ? "cursor-grab active:cursor-grabbing" : "pointer-events-none"
      }`}
      style={{ zIndex: 10 - stackIndex }}
    >
      <div
        className="flex flex-col md:flex-row gap-6 sm:gap-8 p-6 sm:p-8 rounded-2xl border border-white/10 bg-neutral-950 shadow-2xl select-none"
      >
        <div className="w-full md:w-1/3">
          <img
            src={internship.image}
            alt={internship.company}
            className="rounded-xl object-cover w-full h-[180px] sm:h-[220px] md:h-[260px] bg-neutral-900"
            draggable={false}
            loading="lazy"
            decoding="async"
          />
...
const Internships = () => {
  ...
  return (
    <section className="bg-black text-white py-20 sm:py-20 relative overflow-hidden">
      ...
      <div
        className="relative h-[500px] sm:h-[440px] lg:h-[390px] flex justify-center items-center px-4"
